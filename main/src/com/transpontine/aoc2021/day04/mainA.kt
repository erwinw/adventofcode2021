...
private fun process(fileName: String) {
    ...
    guesses.forEachIndexed { guessIdx, chip ->
        ...
        var anyBingo: Int? = null
        boards.forEachIndexed { boardIdx, board ->
            val boardHasBingo = board.hasBingo
            if (boardHasBingo) {
                println("BOARD $boardIdx ${"BINGO".takeIf { boardHasBingo } ?: ""}:\n$board\n")

                val boardSumLeft = board.sumLeft()
                anyBingo = boardSumLeft
            }
        }
        if (anyBingo != null) {
            println("sumLeft $anyBingo x $chip = ${anyBingo!! * chip.toInt()}")
            return
        }
    }
}
